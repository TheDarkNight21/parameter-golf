...
class GPT(nn.Module):
    ...
    def _init_weights(self) -> None:
        ...
        # Init remaining nn.Linear modules (mtp heads, lm_head)
...
def quantize_int6_gptq(weight, hessian=None, clip_range=31, block_size=128):
    ...
    W[:, dead[perm]] = 0
    H = H[perm][:, perm]
    # Cholesky with increasing dampening fallback for numerical stability
    for extra_damp in [0.0, 0.01, 0.1, 1.0]:
        try:
            H_try = H.clone()
            if extra_damp > 0:
                H_try[torch.arange(cols), torch.arange(cols)] += extra_damp * torch.mean(torch.diag(H))
...
def main() -> None:
    ...
    # - mtp heads, VE proj -> Adam (small matrix params not worth banking)
    matrix_params = [
    ...
    torch.cuda.synchronize()
    log0(
        f"final_int6_roundtrip val_loss:{q_val_loss:.4f} "
        f"eval_time:{1000.0 * (time.perf_counter() - t_qeval):.0f}ms"
    )
    log0(f"final_int6_roundtrip_exact val_loss:{q_val_loss:.8f}")
    sw_seq_len = effective_eval_seq_len
    if args.eval_stride > 0 and args.eval_stride < sw_seq_len:
        torch.cuda.synchronize()
        t_slide = time.perf_counter()
        sw_val_loss = eval_val_sliding(
        ...
        torch.cuda.synchronize()
        log0(
            f"final_int6_sliding_window val_loss:{sw_val_loss:.4f} "
            f"stride:{args.eval_stride} eval_time:{1000.0 * (time.perf_counter() - t_slide):.0f}ms"
        )
        log0(f"final_int6_sliding_window_exact val_loss:{sw_val_loss:.8f}")
    if args.eval_stride != 64 and 64 < sw_seq_len:
        torch.cuda.synchronize()
        t_slide64 = time.perf_counter()
        sw64_val_loss = eval_val_sliding(
            args, eval_model, rank, world_size, device,
            val_tokens,
            stride=64,
            eval_seq_len=sw_seq_len,
        ...
        torch.cuda.synchronize()
        log0(
            f"final_int6_sliding_window_s64 val_loss:{sw64_val_loss:.4f} "
            f"stride:64 eval_time:{1000.0 * (time.perf_counter() - t_slide64):.0f}ms"
        )
        log0(f"final_int6_sliding_window_s64_exact val_loss:{sw64_val_loss:.8f}")
    if distributed:
